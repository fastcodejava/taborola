...
function refreshObj() {
    chrome.storage.local.get('googleSearch', function(searchResult){
        ...
        if (location.origin.indexOf('cnn') > -1) {
            ...
            if (queryString === "") {
                $("div.summary").find("h3 > a").each(function (index) {
                    ...
                    if (hrefAdded.indexOf($(this).attr('href')) === -1) {
                        ...
                        /*$(this).parent().next().find("a").each (function (){// code for tree structure
        ...
        }  else if (location.origin.indexOf('craigslist') > -1) {


            //craigslist
            $("div.colmask").find("a").each(function (index) {
                console.log("Title: " + this.text);
                console.log($(this));
                //$(this).append("<br><span style='color: orange'>My new line text</span>");
                console.log('me done');
            ...
            newObj["tree"] = false;
            newObj["sites"] = sites;
            jsonObj['craigslist'] = newObj;
            console.log("Final object...\n" + JSON.stringify(jsonObj));

        }  else if (location.origin.indexOf('nytimes') > -1) {


            //.nytimes
            $("div.span-ab-layout").find("a").each(function (index) {
                ...
                console.log('me done');
                var obj = {};
                if (!(this.parentNode.getAttribute('class') === 'image' || this.parentNode.getAttribute('class') === 'thumb')) {
                    if (this.text.trim() !== '') {
                        if (hrefAdded.indexOf($(this).attr('href')) === -1) {
                            obj[this.text] =  $(this).attr('href');
                            sites.push(obj);
